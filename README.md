...
Work in progress, nothing published yet. Setup, props and ambient types are in
the [package README](packages/vite-material-symbols/README.md).
...
Nx targets are **inferred** by `@nx/vite/plugin`, not declared, so an empty
`targets` block in `project.json` is correct.
